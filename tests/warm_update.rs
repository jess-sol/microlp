...
/// (growth, pivots, further growth — an incremental caller's shape),
/// each step checked against a cold rebuild of the accumulated
/// problem.
#[test]
...
/// A new row constraining a variable that is BASIC (and violated at
/// the current point) must restore feasibility and land on the cold
/// optimum — the hardest warm-apply case for any implementation.
#[test]
fn c_nonzero_row_falls_back_and_matches_cold() {
    ...
    // New row caps the BASIC x at 1.5, violated at the current point —
    // the apply must restore feasibility and re-optimize.
    let mut u = sol.update();
